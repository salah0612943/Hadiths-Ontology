<%@ WebHandler Language="C#" Class="pagetext" %>

using System;
using System.Web;
using System.Data;

public class pagetext : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        try
        {
            ...
            string txtname = (context.Request["txtname"] != null) ? context.Request["txtname"] : "";
            ...
            if (mode == "add")
            {
                sql = "Insert into pagetext(ar,en,txtname,pageid) Values(N'" + name + "','" + ename + "','" + txtname + "','" + id + "')";
                if (db.Execute(sql))
                {
            ...
            if (mode == "edit")
            {
                sql = "Update  pagetext set ar=N'" + name + "' ,en='" + ename + "' where pageid='" + id + "' and txtname='" + txtname + "'";
                if (db.Execute(sql))
                {
            ...
            if (mode == "delete")
            {
                sql = "Delete From   pagetext  where pageid='" + id + "' and txtname='"+txtname+"'";
                if (db.Execute(sql))
                {
            ...
            if (sename != "") condition += " and en like '%" + sename + "%'";
            ////========================= sort option
            string orderby = " order by txtname";
            if (sort != "") orderby = " order by " + sort + "  " + sorttype; 
                
            //---------------- find all rows
            sql = "Select isnull(count(pageid),0) as no from pagetext " + condition;
