<%@ WebHandler Language="C#" Class="groupinfo" %>

using System;
using System.Web;
using System.Data;

public class groupinfo : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        try
        {
            ...
            if (mode == "add")
            {
                sql = "Insert into groupinfo(namear,nameen) Values(N'" + name + "','" + ename + "')";
                if (db.Execute(sql))
                {
            ...
            if (mode == "edit")
            {
                sql = "Update  groupinfo set namear=N'" + name + "' ,nameen='" + ename + "' where groupid='" + id + "'";
                if (db.Execute(sql))
                {
            ...
            if (mode == "delete")
            {
                sql = "Delete From   groupinfo  where groupid='" + id + "'";
                if (db.Execute(sql))
                {
            ...
            ////-------------- Search Condition
            string condition = " where groupid!=0";
            if (sid != "") condition += " and groupid like '%" + sid + "%'";
            if (sname != "") condition += " and namear like N'%" + sname + "%'";
            if (sename != "") condition += " and nameen like '%" + sename + "%'";
            ////========================= sort option
            string orderby =  " order by groupid";
            if (sort != "") orderby = " order by " + sort + "  " + sorttype; 
                
            //---------------- find all rows
            sql = "Select isnull(count(groupid),0) as no from groupinfo " + condition;
            ...
            sql += " groupid,namear ,nameen  FROM groupinfo " + condition + ") ";
