...
function showinfo(mode) {
    ...
    try {
        ...
        var id = document.getElementById('cmbm').value;
        ...
        $.ajax({
            type: "GET",
            url: "code/pagetext.ashx",
            data: {txtname:txtname, id: id, name: name, ename: ename, sname: sname, sename: sename, mode: mode, page: page, rows: rows, sort: sort, order: order },
...
function showdata(no) {
    try {
        ...
        document.getElementById('id').value = obj.rows[no].txtname;
        document.getElementById('data').style.display = 'block';
        var x = document.getElementById('btndelete');
        if (x.className.indexOf("w3-hide") != -1) {
